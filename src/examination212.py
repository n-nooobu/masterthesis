...
train_df, train_prevm = batch_divide(max_tap=max_tap, batch_size=batch_size1, start=0, target_num=target_num1, target_dir='train_0_8B10B_equalize')
...
np.savetxt('result212_001.csv', result, delimiter=',')

tap_list = [i * 5 + (1 - i % 2) for i in range(20)]
for i, tap in enumerate(tap_list):
    result = np.loadtxt('result212_001.csv', delimiter=',')
    retsult = train(i, tap, result)
    np.savetxt('result212_001.csv', retsult, delimiter=',')
    time.sleep(10)
